...
export class IFrameWindow extends AbstractChildWindow {
  ...
  public constructor({
    ...
    this._frame = IFrameWindow.createHiddenIframe(this._parent);
    this._window = this._frame.contentWindow;
  }

  private static createHiddenIframe(_parent?: any): HTMLIFrameElement {
    const iframe = window.document.createElement("iframe");

    // iframe.style.position = "absolute";
    // iframe.style.left = "0";
    // iframe.style.top = "0";
    iframe.className = "rounded-lg w-full h-full absolute left-0 top-0";
    iframe.width = "100%";
    iframe.height = "100%";
    iframe.setAttribute(
      "sandbox",
